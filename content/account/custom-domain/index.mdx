...
A custom domain lets your Miwa.lol profile appear on a domain you own. For example, if you own `example.com`, visiting it would display your profile. This page explains how to set one up. Don't worry, it's relatively simple.
...
* A domain name (obviously, this guide won't help much without one!)<br/>
...
<Steps>
    ...
    2. In your domain's DNS settings, create a CNAME record pointing to `miwa.lol`.
...
Some DNS providers don't allow a CNAME record at the domain root (the apex). To avoid this and similar DNS issues, we recommend using [Cloudflare](https://www.cloudflare.com/)'s nameservers.